...
public class DescriptionDAOImpl extends AbstractPersistableDAO<Description> {

  public DescriptionDAOImpl() {
    super(Description.class, true);
  }

  public Description load(long fNewsId) {
    Query query = fDb.query();
    query.constrain(Description.class);
    query.descend("fNewsId").constrain(fNewsId);
    Description description = getSingleResult(query);
    fDb.activate(description, Integer.MAX_VALUE);
    return description;
  }

  public String loadValue(long fNewsId) {
    Description description = load(fNewsId);
